...
function adicionarRelatorio() {
    ...
    if (toggle) {
        toggle.setAttribute('onclick', `toggleValores(${relatorioIndex})`);
    }
...
// Busca os dados do contrato ao selecionar
        ...
        fetch(`/api/contrato/${encodeURIComponent(contrato)}`)
            ...
            .then(data => {
                ...
                const map = {
                    ...
                    "EMPRESA": "EMPRESA",
                    "ESTADO": "ESTADO",
                    "PRAZO FINAL CONTRATUAL": "PRAZO FINAL CONTRATUAL",
                    "VALOR DO CONTRATO": "VALOR DO CONTRATO",
                    "VALOR ADITIVO": "VALOR ADITIVO",
                    "VALOR CONTRATO + ADITIVOS": "VALOR CONTRATO + ADITIVOS",
                    "VALOR CONSUMIDO": "VALOR CONSUMIDO",
                    "SALDO DO CONTRATO": "SALDO DO CONTRATO",
                    "SALDO DO CONTRATO PERCENTUAL": "SALDO DO CONTRATO PERCENTUAL"
                };

                Object.entries(map).forEach(([chave, campo]) => {
                    const input = form.querySelector(`[name="${campo}"]`);
                    if (input) input.value = data[chave] || "";
                });
            });
    }
});
